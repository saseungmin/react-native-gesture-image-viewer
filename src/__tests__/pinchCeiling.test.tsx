...
async function setup(maxZoomScale = 4, contentHeight = 852) {
  ...
  return {
    ...
    readTransform,
    lockPaging: () => hook.result.current.horizontalPagingGesture.handlers.onStart?.({} as never),
  };
}
...
describe('pinch ceiling focal preservation', () => {
  ...
  it('does not settle a stale pinch while a page transition owns the transform', async () => {
    const viewer = await setup();
    await act(async () => {
      viewer.start(96.5, 326);
      viewer.update(5, 96.5, 326);
    });
    await act(async () => viewer.lockPaging());
    const before = viewer.readTransform();
    jest.mocked(Reanimated.withTiming).mockClear();
    await act(async () => viewer.end());
    expect(viewer.readTransform()).toEqual(before);
