...
import type { PanGesture, PinchGesture, TapGesture } from 'react-native-gesture-handler';
...
describe('rotation bounds lifecycle', () => {
  ...
  it('clamps a rotated pinch, its ceiling settlement, and subsequent pan', async () => {
    ...
    const pinch = viewer.hook.result.current.zoomGesture
      .toGestureArray()
      .find((gesture) => gesture.handlerName === 'PinchGestureHandler') as PinchGesture;
    await act(async () => {
  ...
  it('preserves the v2 transform reset when resizing during zoom', async () => {
    const viewer = await setup();
    await act(async () => viewer.rotation.set(90));
    const doubleTap = viewer.hook.result.current.zoomGesture
      .toGestureArray()
  ...
  it('does not let a cancelled rotation overwrite the next page reset', async () => {
    ...
    viewer.manager.setListRef({ scrollToOffset: jest.fn() });
    await act(async () => viewer.manager.goToIndex(1));
    await act(async () => jest.advanceTimersByTime(500));
    expect(viewer.read()).toEqual({ x: 0, y: 0, rotation: 0 });
    expect(viewer.values.scale.get()).toBe(1);
  });
