...
function App() {
  ...
  return <GestureViewer enableLoop={true} />;
}
```
...
function App() {
  ...
  return <GestureViewer onIndexChange={setCurrentIndex} />;
}
```
...
function App() {
  return <GestureViewer data={data} renderItem={renderItem} useSnap={true} />;
}
```
...
function App() {
  return <GestureViewer autoPlay={true} />;
}
```
...
function App() {
  return <GestureViewer autoPlay={true} autoPlayInterval={3000} />;
}
```
