...
### Item dimensions for zoom bounds

The third argument also provides `setItemDimensions({ width, height })`; call it from an image load or event callback, or from a passive `useEffect` after commit. Do not call it during render or from a descendant layout effect. When dimensions are already known, `getItemDimensions(item, index)` is the eager alternative. A valid runtime registration takes precedence over the getter; when neither is available, the viewer uses the full cell. Known natural dimensions are centered with contain fitting before zoom bounds are calculated. If the same logical item may be recreated as a new object in the same slot, add `getItemKey(item, index)` so those runtime dimensions stay attached to that slot.

### Stable keys for runtime dimensions

`getItemKey(item, index)` returns a stable logical key for the item in that slot. Use it when runtime dimensions are reported for a slot that may be recreated as a new object. The key must identify the same rendered content and change when the rendered source or natural dimensions change. Do not return the array index alone. If the item changes position, the viewer falls back to the getter or to an undefined content size until the new slot reports dimensions again.
