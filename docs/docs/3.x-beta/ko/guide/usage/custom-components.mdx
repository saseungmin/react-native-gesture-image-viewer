...
런타임 치수가 필요하다면 렌더된 컴포넌트에서 load 이후 `setItemDimensions`를 호출하세요. 같은 논리 아이템이 같은 slot에서 다른 객체로 다시 만들어질 수 있다면 `getItemKey`를 함께 사용해 runtime 치수가 그 slot에 유지되도록 하세요.
