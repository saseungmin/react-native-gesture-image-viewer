...
세 번째 인자는 `setItemDimensions({ width, height })`도 제공합니다. 이미지 load callback에서 호출하고 렌더 중에는 호출하지 마세요. 크기를 이미 알고 있다면 `getItemDimensions(item, index)`를 사용할 수 있습니다. 유효한 runtime 등록이 getter보다 우선하며, 둘 다 없으면 viewer 전체 cell 크기를 사용합니다. 알려진 원본 크기는 centered contain fitting 뒤 zoom bounds에 적용됩니다.
