...
세 번째 인자는 `setItemDimensions({ width, height })`도 제공합니다. 이미지 load 또는 event callback에서 호출하거나, commit 이후 passive `useEffect`에서 호출하세요. 렌더 중이나 descendant layout effect에서는 호출하지 마세요. 크기를 이미 알고 있다면 `getItemDimensions(item, index)`를 사용할 수 있습니다. 같은 논리 아이템이 다른 객체로 다시 만들어질 수 있다면 `getItemKey(item, index)`를 추가해 runtime 치수가 그 slot에 유지되도록 하세요. 유효한 runtime 등록이 getter보다 우선하며, 둘 다 없으면 viewer 전체 cell 크기를 사용합니다. 알려진 원본 크기는 centered contain fitting 뒤 zoom bounds에 적용됩니다.

### runtime 치수를 위한 안정적인 key

`getItemKey(item, index)`는 해당 slot의 아이템에 대한 안정적인 논리 key를 반환합니다. 같은 아이템이 다른 객체로 다시 만들어질 수 있을 때 사용하세요. 이 key는 같은 렌더 결과를 가리켜야 하며, 렌더된 source나 원본 치수가 바뀌면 함께 바뀌어야 합니다. 배열 인덱스만 그대로 반환하지 마세요. 아이템이 다른 위치로 이동하면, 새 slot이 치수를 다시 보고할 때까지 getter 또는 undefined content size로 fallback합니다.
...
export interface GestureViewerProps<ItemT> {
  ...
  /**
   * 같은 아이템이 다른 객체로 다시 만들어질 수 있는 같은 slot에서 사용할 안정적인 논리 key를 반환합니다.
   * @remarks rerender마다 객체 identity가 안정적이지 않을 때 사용하세요. 이 key는 같은 렌더 결과를 가리켜야 하며, 렌더된 source나 원본 치수가 바뀌면 함께 바뀌어야 합니다. 배열 인덱스만 그대로 반환하지 마세요. 아이템이 다른 위치로 이동하면, 새 slot이 치수를 다시 보고할 때까지 getter 또는 undefined content size로 fallback합니다.
   */
