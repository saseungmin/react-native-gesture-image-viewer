...
| 속성           | 설명                               | 타입                                                        | 파라미터 기본값 |
| :------------- | :--------------------------------- | :---------------------------------------------------------- | :-------------: |
| `goToIndex`    | 특정 인덱스로 이동합니다.          | `(index: number, options?: { animated?: boolean }) => void` |        -        |
...
페이지 전환 애니메이션 없이 현재 아이템을 즉시 바꿔야 한다면 `goToIndex(index, { animated: false })`를 사용하세요.
v3에서는 인접한 이동(`goToPrevious`, `goToNext`, 또는 이전/다음 아이템으로 이동하는 `goToIndex`)에만 페이지 전환 애니메이션을 사용합니다. 인접하지 않은 `goToIndex` 호출은 작은 render window만 마운트하는 구조이므로 즉시 rebase됩니다.
