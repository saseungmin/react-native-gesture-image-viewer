...
On release, the `horizontalSwipe` distance and velocity thresholds decide whether the page turns, measured on the drag past the edge only. A flick back toward the item never turns the page the other way. When the page turns, the outgoing item is reset once it is off screen and the incoming item arrives at its fitted scale; otherwise the page settles back and the item stays zoomed at its edge. The first and last items resist without `enableLoop`. A second finger or a cancelled pan settles the page back. While a handoff is in progress, the page owns horizontal release momentum, so `panInertia` applies only vertically. Handoff is not available while the item is rotated. Supply `getItemDimensions` or `setItemDimensions` so the edge is the content's own; otherwise the viewer dimensions are used.
