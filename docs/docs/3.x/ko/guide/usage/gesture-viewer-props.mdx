...
손을 떼면 가장자리를 지난 드래그만을 기준으로 `horizontalSwipe`의 거리·속도 기준에 따라 페이지 전환 여부를 정합니다. 아이템 쪽으로 되돌리는 플릭은 반대 방향으로 페이지를 넘기지 않습니다. 페이지가 넘어가면 이전 아이템은 화면 밖으로 나간 뒤 초기화되고 새 아이템은 맞춤 크기로 표시됩니다. 넘어가지 않으면 페이지는 제자리로 돌아오고 아이템은 가장자리에서 확대 상태를 유지합니다. `enableLoop`가 꺼져 있으면 첫 번째와 마지막 아이템에서 저항이 적용됩니다. 두 번째 손가락이 닿거나 드래그가 취소되면 페이지는 제자리로 돌아옵니다. 페이지 전환 중에는 가로 관성을 페이지가 처리하므로 `panInertia`는 세로 방향에만 적용됩니다. 아이템이 회전된 상태에서는 동작하지 않습니다. 실제 콘텐츠 가장자리를 사용하려면 `getItemDimensions` 또는 `setItemDimensions`로 크기를 제공하세요. 없으면 뷰어 크기를 기준으로 합니다.
