...
`renderItem`은 `data`의 각 아이템을 렌더링하며, `renderItem(item, index, { isActive, setItemDimensions })` 형태로 세 번째 인자에서 `isActive`와 `setItemDimensions`를 제공합니다.

비디오 재생처럼 활성 아이템에서만 필요한 작업은 `isActive`로 제어할 수 있습니다. 동작 방식과 예시는 [`renderItem` 활성 상태 (`isActive`)](/ko/guide/usage/custom-components.html#renderitem-active-state)를 참고하세요.

`setItemDimensions({ width, height })`는 렌더링된 콘텐츠의 원본 크기를 알 수 있게 된 뒤 호출하세요. 예시와 호출 시점은 [콘텐츠 크기 가이드](/ko/guide/usage/item-dimensions.html)를 참고하세요.

### `panInertia` (기본값: `false`)

확대한 이미지를 드래그하고 손을 떼면 관성으로 이동하다 감속합니다. 기본 설정은 `true`, 세부 설정은 `enabled`를 명시한 객체로 전달합니다.
...
생략하거나 `false`, `{ enabled: false }`를 전달하면 기존처럼 관성 없이 멈춥니다. 빈 객체는 허용하지 않으며 `enablePanWhenZoomed`도 켜져 있어야 합니다.

필요한 값만 지정하면 생략한 항목에는 다음 기본값을 사용합니다.

| 옵션               | 기본값   | 동작                                                                                                         |
| ------------------ | -------- | ------------------------------------------------------------------------------------------------------------ |
| `deceleration`     | `0.9997` | `1`에 가까울수록 오래 미끄러집니다.                                                                          |
| `velocityFactor`   | `0.5`    | 같은 드래그 속도에 대한 이동량을 조절합니다.                                                                 |
| `rubberBandEffect` | `true`   | 경계를 잠깐 넘었다가 돌아옵니다. `false`이면 경계에서 바로 멈춥니다.                                         |
| `rubberBandFactor` | `1`      | 경계를 넘은 뒤 돌아오는 힘입니다. 값이 클수록 더 강하게 돌아와 덜 튀어나갑니다. 탄성을 켰을 때만 적용됩니다. |

`deceleration`은 `0`보다 크고 `1`보다 작은 유한한 값이어야 합니다. `velocityFactor`와 `rubberBandFactor`는 `0`보다 큰 유한한 값이어야 합니다. 잘못된 값은 각 항목의 기본값으로 대체합니다. 설정 변경은 다음 드래그 종료부터 적용하며, 관성을 비활성화하면 현재 애니메이션을 중단하고 경계를 넘은 위치도 정리합니다.

탄성은 손을 뗀 뒤의 관성 이동에 적용됩니다. 콘텐츠 경계 안에서 멈추며 페이지를 전환하지 않습니다. 실제 콘텐츠 경계를 사용하려면 `getItemDimensions` 또는 `setItemDimensions`로 크기를 제공하세요. 없으면 뷰어 크기를 기준으로 제한합니다. 새 터치는 경계 밖이라도 잡은 위치를 유지하고, 손을 떼면 경계로 돌아옵니다. 핀치, 확대·회전·크기 변경, 페이지 이동, 닫기도 관성을 중단합니다. 실제 드래그 속도와 경계, 시스템의 동작 줄이기 설정은 뷰어 내부에서 관리하며 `panInertia`로 변경할 수 없습니다.

### `edgeHandoffPaging` (기본값: `false`)

iOS 사진 앱처럼 확대한 이미지를 드래그하다 가장자리에 닿으면 같은 드래그로 페이지를 넘깁니다. 확대한 아이템의 가로 가장자리가 뷰어 가장자리에 닿은 뒤의 드래그는 버려지지 않고 페이지를 이동합니다. 기본 설정은 `true`, 세부 설정은 `enabled`를 명시한 객체로 전달합니다.
...
생략하거나 `false`, `{ enabled: false }`를 전달하면 기존처럼 확대 상태의 드래그가 가장자리에서 멈춥니다. `enablePanWhenZoomed`와 `horizontalSwipe.enabled`도 켜져 있어야 합니다.

| 옵션        | 기본값 | 동작                                                                          |
| ----------- | ------ | ----------------------------------------------------------------------------- |
| `threshold` | `0`    | 가장자리를 지난 뒤 페이지가 손가락을 따라오기 시작하는 드래그 거리(pt)입니다. |

`threshold`는 `0` 이상의 유한한 값이어야 하며, 잘못된 값은 기본값으로 대체합니다.

세로보다 가로로 더 많이 움직인 드래그에서만 전환이 시작되므로, 옆으로 조금 흔들린 세로 드래그는 계속 아이템을 이동합니다. 드래그가 아이템 안쪽으로 돌아오면 전환이 끝납니다. 손을 떼면 `horizontalSwipe` 기준으로 페이지 전환 여부를 정합니다. 거리 기준은 가장자리와 `threshold`를 지난 드래그에, 속도 기준은 가로가 세로보다 빠른 플릭의 속도에만 적용합니다. 아이템 쪽으로 되돌리는 플릭은 반대 방향으로 페이지를 넘기지 않습니다. 페이지가 넘어가면 이전 아이템은 화면 밖으로 나간 뒤 초기화되고 새 아이템은 맞춤 크기로 표시됩니다. 넘어가지 않으면 페이지는 제자리로 돌아오고 아이템은 가장자리에서 확대 상태를 유지합니다. `enableLoop`가 꺼져 있으면 첫 번째와 마지막 아이템에서 저항이 적용됩니다. 두 번째 손가락이 닿거나 드래그가 취소되면 페이지는 제자리로 돌아옵니다. `panInertia`를 켜면 아이템 쪽으로 되돌리는 플릭은 페이지가 제자리로 돌아오는 동안에도 아이템을 관성으로 이동합니다. 가장자리 밖으로 손을 떼면 아이템의 가로 관성은 적용하지 않습니다. 아이템이 회전된 상태에서는 동작하지 않습니다. 실제 콘텐츠 가장자리를 사용하려면 `getItemDimensions` 또는 `setItemDimensions`로 크기를 제공하세요. 없으면 뷰어 크기를 기준으로 합니다.

### `getItemDimensions`

`getItemDimensions(item, index)`는 콘텐츠의 원본 크기를 이미 알고 있을 때 반환합니다. 우선순위와 대체 동작은 [콘텐츠 크기 가이드](/ko/guide/usage/item-dimensions.html)를 참고하세요.

### `getItemKey`

`getItemKey(item, index)`는 같은 논리 아이템이 같은 슬롯에서 다른 객체로 다시 만들어질 수 있을 때, 런타임에 등록한 크기를 유지하기 위한 안정적인 키를 반환합니다. 키 조건은 [콘텐츠 크기 가이드](/ko/guide/usage/item-dimensions.html#getitemkey)를 참고하세요.
...
export type GestureViewerItemDimensions = Readonly<{
  /** 콘텐츠의 원본 너비입니다. 유한한 양수여야 합니다. */
  width: number;
  /** 콘텐츠의 원본 높이입니다. 유한한 양수여야 합니다. */
  height: number;
}>;
...
export type GestureViewerRenderItemInfo = {
  ...
  /**
   * 렌더링된 콘텐츠의 원본 크기를 사용할 수 있게 된 후 등록합니다.
   * @remarks 이미지 로드 또는 이벤트 콜백에서 호출하거나, 커밋 이후의 일반 `useEffect`에서 호출하세요. 렌더링 중이나 하위 컴포넌트의 `useLayoutEffect`에서는 호출하지 마세요.
   */
...
export interface GestureViewerProps<ItemT> {
  ...
  /**
   * 콘텐츠의 원본 크기를 이미 알고 있을 때 반환합니다.
   * @remarks 크기를 아직 모르면 `undefined`를 반환하세요. 유효하지 않은 크기는 뷰어 셀 크기로 대체됩니다.
   */
  getItemDimensions?: GestureViewerItemDimensionsResolver<ItemT>;
  /**
   * 같은 아이템이 같은 슬롯에서 다른 객체로 다시 만들어질 수 있을 때 사용할 안정적인 논리 키를 반환합니다.
   * @remarks 객체 동일성이 리렌더링 사이에 유지되지 않을 때 사용하세요. 이 키는 같은 렌더 결과를 가리켜야 하며, 이미지 소스나 원본 크기가 바뀌면 함께 바뀌어야 합니다. 배열 인덱스만 그대로 반환하지 마세요. 아이템이 다른 위치로 이동하면 새 슬롯에서 크기를 다시 등록할 때까지 `getItemDimensions` 또는 뷰어 셀 크기를 사용합니다.
   */
  ...
  /**
   * 확대된 이미지를 드래그하고 손을 떼면 관성을 적용합니다.
   * @remarks `enablePanWhenZoomed`가 활성화되어 있어야 합니다. 기본 설정은 `true`, 사용자 설정은 `enabled: true`를 포함한 객체로 전달합니다. 콘텐츠 경계와 시스템의 동작 줄이기 설정을 따릅니다.
   * @defaultValue false
   */
  panInertia?:
    | boolean
    | {
        /** 설정 객체를 사용할 때 관성 활성화 여부를 명시합니다. */
        enabled: boolean;
        /**
         * 속도가 줄어드는 정도입니다. `1`에 가까울수록 오래 미끄러집니다.
         * @remarks `0`보다 크고 `1`보다 작은 유한한 값만 허용합니다. 잘못된 값은 기본값으로 대체합니다.
         * @defaultValue 0.9997
         */
        deceleration?: number;
        /**
         * 손을 뗄 때의 속도에 적용해 이동량을 조절하는 배율입니다.
         * @remarks `0`보다 큰 유한한 값만 허용합니다. 잘못된 값은 기본값으로 대체합니다.
         * @defaultValue 0.5
         */
        velocityFactor?: number;
        /**
         * 콘텐츠 경계를 잠깐 넘었다가 가장자리로 돌아오는 탄성을 적용합니다.
         * @defaultValue true
         */
        rubberBandEffect?: boolean;
        /**
         * 경계를 넘은 뒤 돌아오는 힘의 크기입니다. `rubberBandEffect`가 false이면 무시합니다.
         * @remarks `0`보다 큰 유한한 값만 허용합니다. 잘못된 값은 기본값으로 대체합니다.
         * @defaultValue 1
         */
        rubberBandFactor?: number;
  ...
  /**
   * 확대한 드래그가 가로 가장자리에 닿으면 이어서 페이지를 넘깁니다.
   * @remarks `enablePanWhenZoomed`와 `horizontalSwipe.enabled`가 필요합니다. 기본 설정은 `true`, 세부 설정은 `enabled: true`를 포함한 객체로 전달합니다. 손을 뗄 때 `horizontalSwipe` 기준으로 전환 여부를 정하며, 새 아이템은 맞춤 크기로 표시됩니다.
   * @defaultValue false
   */
  edgeHandoffPaging?:
    | boolean
    | {
        /** 설정 객체를 사용할 때 가장자리 페이지 전환을 명시적으로 켜거나 끕니다. */
        enabled: boolean;
        /**
         * 가장자리를 지난 뒤 페이지가 손가락을 따라오기 시작하는 드래그 거리(pt)입니다.
         * @remarks `0` 이상의 유한한 값만 허용합니다. 잘못된 값은 기본값으로 대체합니다.
         * @defaultValue 0
         */
        threshold?: number;
      };
