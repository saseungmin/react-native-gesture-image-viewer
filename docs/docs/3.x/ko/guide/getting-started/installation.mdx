...
Expo SDK와 호환되는 버전은 [Expo Reanimated 설치 가이드](https://docs.expo.dev/versions/latest/sdk/reanimated/)에 따라 설치하세요.

<PackageManagerTabs command="expo install react-native-reanimated react-native-worklets" />

`babel-preset-expo`가 Babel 플러그인을 자동으로 구성합니다. Expo Go에서는 Reanimated 4가 포함된 Expo SDK를 사용하세요.
...
이제 `react-native-gesture-image-viewer`의 정식 버전을 설치하면 됩니다.
