...
리스트 기반 페이징이 어떻게 바뀌었는지 궁금하다면 개요의 [v3 페이징 동작 방식](/ko/guide/getting-started/overview.html#v3-페이징-동작-방식)을 확인하세요. 렌더링되는 페이지와 페이지 전환 시 render window가 갱신되는 과정을 그림과 함께 설명합니다.
