...
아이템의 원본/source 치수를 이미 알고 있을 때 반환합니다. 데이터에 `width`, `height`가 있는 `contain` 이미지에서 사용하면 팬과 더블 탭 줌 범위를 실제 콘텐츠 가장자리에 맞춰 계산할 수 있습니다.

치수를 아직 모르면 `undefined`를 반환하세요. 이후 `renderItem`에서 `setItemDimensions`를 호출하면 런타임에 등록된 치수가 우선됩니다.

```tsx
const getImageDimensions = (image: (typeof images)[number]) => ({
  width: image.width,
  height: image.height,
});
...
같은 내용이 같은 slot에서 다른 객체로 다시 만들어질 수 있을 때 사용할 안정적인 논리 키를 반환합니다. 이 키를 사용하면 같은 slot의 런타임 치수를 rerender 사이에서 재사용할 수 있습니다. 이 키는 같은 렌더 결과를 가리켜야 하며, 렌더된 source나 원본 치수가 바뀌면 함께 바뀌어야 합니다. 배열 인덱스만 그대로 반환하지 마세요. 아이템이 다른 위치로 이동하면, 새 slot이 치수를 다시 보고할 때까지 getter 또는 undefined content size로 fallback합니다.

```tsx
const getImageKey = (image: (typeof images)[number]) => image.uri;
...
export type GestureViewerItemDimensions = Readonly<{
  /** 원본/source 콘텐츠 너비입니다. 유한한 양수여야 합니다. */
  width: number;
  /** 원본/source 콘텐츠 높이입니다. 유한한 양수여야 합니다. */
  height: number;
}>;
...
export type GestureViewerRenderItemInfo = {
  ...
  /**
   * 렌더링된 아이템의 원본/source 치수를 사용할 수 있게 된 후 등록합니다.
   * @remarks 이미지 load 또는 event 콜백에서 호출하거나, commit 이후 passive `useEffect`에서 호출하세요. 렌더 중이나 descendant layout effect에서는 호출하지 마세요.
   */
...
export interface GestureViewerProps<ItemT, LC> {
  ...
  /**
   * 아이템의 원본/source 치수를 이미 알고 있을 때 반환합니다.
   * @remarks 치수를 아직 모르면 `undefined`를 반환하세요. 잘못된 치수는 뷰어 셀 크기 fallback으로 처리됩니다.
   */
  getItemDimensions?: GestureViewerItemDimensionsResolver<ItemT>;
  /**
   * 같은 아이템이 같은 slot에서 다른 객체로 다시 만들어질 수 있을 때 사용할 안정적인 논리 키를 반환합니다.
   * @remarks 객체 identity가 rerender마다 안정적이지 않을 때 사용하세요. 이 키는 같은 렌더 결과를 가리켜야 하며, 렌더된 source나 원본 치수가 바뀌면 함께 바뀌어야 합니다. 배열 인덱스만 그대로 반환하지 마세요. 아이템이 다른 위치로 이동하면, 새 slot이 치수를 다시 보고할 때까지 getter 또는 undefined content size로 fallback합니다.
   */
