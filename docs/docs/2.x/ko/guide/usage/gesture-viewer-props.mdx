...
<GestureViewer
  data={data}
  renderItem={renderItem}
  panInertia={{ enabled: true, deceleration: 0.995 }} // [!code highlight]
/>
```
...
`deceleration` 기본값은 `0.998`이며 `1`에 가까울수록 오래 미끄러집니다. `0`보다 크고 `1`보다 작은 유한한 값만 허용하며 잘못된 값은 기본값으로 대체합니다. 감속률 변경은 다음 드래그 종료부터 적용하고, 관성을 비활성화하면 진행 중인 움직임도 멈춥니다.

이미지 경계에서 멈추며 탄성 효과나 페이지 전환은 포함하지 않습니다. 실제 콘텐츠 경계를 사용하려면 `getItemDimensions` 또는 `setItemDimensions`로 크기를 제공하세요. 없으면 뷰어 크기를 기준으로 제한합니다. 새 터치·핀치, 확대·회전·크기 변경, 페이지 이동, 닫기는 관성을 중단합니다. 시스템의 동작 줄이기 설정도 따릅니다.
...
export interface GestureViewerProps<ItemT, LC> {
  ...
  panInertia?:
    ...
    | {
        ...
        enabled: boolean;
        /**
         * 손을 뗀 뒤 이동 속도가 줄어드는 정도를 조절합니다. `1`에 가까울수록 오래 미끄러집니다.
