...
| `velocityFactor`   | `0.65`   | Scales travel for the same release velocity.                                                             |
| `rubberBandEffect` | `true`   | Briefly overshoots the edge, then returns. Set `false` for a hard stop.                                  |
| `rubberBandFactor` | `2`      | Return strength: higher values pull back more firmly with less overshoot; only used with rubber-banding. |
...
Returns natural/source dimensions for an item when they are already known. Runtime dimensions reported from `renderItem` with `setItemDimensions` take precedence. See the [item dimensions guide](/guide/usage/item-dimensions.html) for examples.

### `getItemKey`

Returns a stable logical key for an item when the same content may be recreated as a new object in the same slot. Use it with runtime `setItemDimensions` values. See the [item dimensions guide](/guide/usage/item-dimensions.html#getitemkey) for details.
