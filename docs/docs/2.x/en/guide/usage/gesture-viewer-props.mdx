...
Returns natural/source dimensions for an item when they are already known. Use this for `contain` images whose data already includes `width` and `height`; the viewer uses these values to calculate accurate pan and double-tap zoom bounds.

Return `undefined` while dimensions are unavailable. If `renderItem` later calls `setItemDimensions`, the registered runtime dimensions take precedence.
...
Returns a stable logical key for an item when the same content may be recreated as a new object in the same slot. The viewer uses this key to reuse runtime dimensions for that slot across rerenders. The key must identify the same rendered content and change when the rendered source or natural dimensions change. Do not return the array index alone. If the item changes position, the viewer falls back to the getter or to an undefined content size until the new slot reports dimensions again.
