...
`deceleration` defaults to `0.998`; values closer to `1` glide longer. Only finite values strictly between `0` and `1` are accepted; invalid values fall back to the default. Changing it affects the next release. Disabling inertia stops any running glide.

Movement stops at the fitted content bounds without rubber-banding or changing pages. Supply `getItemDimensions` or `setItemDimensions` for content-aware bounds; otherwise the viewer dimensions are used. A new touch/pinch or changes to zoom, rotation, geometry, navigation, or dismissal interrupt inertia. System reduced-motion preferences are respected.
...
export interface GestureViewerProps<ItemT, LC> {
  ...
  panInertia?:
    ...
    | {
        ...
        enabled: boolean;
        /**
         * Controls how quickly the release velocity decays. Values closer to `1` glide longer.
         * @remarks Accepts finite values strictly between `0` and `1`. Invalid values fall back to the default.
         * @defaultValue 0.998
         */
        deceleration?: number;
      };
